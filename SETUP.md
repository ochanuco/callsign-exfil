...
対象バージョン: **UE 5.7.4** (Apple Silicon ネイティブ、Lumen/Nanite が Metal で動作する世代)。  
Xcode の必要バージョンは UE 5.7 系のリリースノートで確認すること (近年のリリースは Xcode 16 系が前提)。
...
#
# 3-3. 左ペイン "Unreal Engine" → "Library" → "+" で UE 5.7.4 をインストール
...
```bash
ls /Users/Shared/Epic\ Games/UE_5.7/Engine/Build/BatchFiles/Mac/
...
1. Epic Games Launcher → "Unreal Engine" → "Library" → 5.7.4 の "Launch"
...
```bash
# 以下のパスは例示。実際のクローン先パスに置き換えること。
cd <your-repo-path>/game/CallsignExfil
...
| Xcode のバージョン不一致でビルドが壊れる | UE 5.7 系のリリースノートで指定された Xcode のメジャーバージョンに合わせる |
