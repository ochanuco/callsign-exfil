...
`FCallsignShotRequest` の Phase 1 シグネチャは破壊しない。Inventory 参照は新規フィールドとして追加し、未指定時は Phase 1 互換 (耐久・弾薬を消費しない) で動作する。**ただし二系統共存は Phase 2 完了時点で解消する**: Phase 2 機能が main に乗ったタイミングで Phase 1 経路 (Inventory 未指定経路) を deprecate し、Phase 3 着手前に削除する (詳細は §Consequences/Negative および §Risks 参照)。
...
- `UCallsignInventoryComponent::Reload()` の内部分岐:
  - 現在武器が `bIsRescue=true` (= `bHasInfiniteMagazine=true`) の場合は no-op (リロード不要)
  - **所持弾薬が 0 の場合は no-op** — `FCallsignReloadResult.bReloaded=false` を返し `OnReloaded` は broadcast しない。UI 側は `IsReloadAvailable()` 事前チェックでボタンを無効化する想定。ターン消費は PC 側で `bReloaded=false` を見て発生させない (空打ちでターン消費しない)
...
**`WearLevel` のみ意図的な例外** とする。Phase 3 ニコイチで `WearLevel++` を扱うが、その時点で `FCallsignWeaponInstance` の構造を変えると Phase 2 で永続化された武器インスタンスのデータ移行コストが発生する。この移行コストを Phase 2 段階で先払いすることを優先し、`WearLevel int32` だけは Phase 2 から枠を確保する。Phase 2 では値を 0 に固定し参照しない。本例外は他のフィールドには適用しない。

### 13. Open Questions

Phase 2 実装中に検証・確定が必要な点。**確定済み判断は本セクションに置かない** (確定済みは §Decision に記載、Phase 3 で覆る場合の移行コストは §Consequences/Risks に記載する)。
...
2. 救済ハンドガンを別スロット (`Rescue`) として enum に持つ方針は採用したが、UI のスロット表示で 5 枠並べるか 4+常駐表記にするかは UMG 設計時に決める。
3. リロード/破損のアニメ通知 (delegate broadcast の購読側 UMG/SE) を Phase 2 で実装するか、Phase 3 まで TODO に残すか。Phase 2 完了条件 (`PLAN.md`) はゲームプレイ判断が成立すれば足りるため、最低限のテキストログ/HUD 通知に留めて Phase 3 で見栄えを整える方針を提案する。

確定済み判断の参照先:

- Inventory の所有者 (Pawn 上 `UActorComponent` 採用): §4.1。Phase 3 で覆る場合の移行コストは §Risks 参照。
- 弾薬プールの所有者 (プレイヤー Pawn ごと、敵側は弾薬無限扱い): §11 + §4.1。Phase 3 で覆る場合の移行コストは §Risks 参照。
...
- 救済ハンドガンの `bIsRescue=true` 経路が Combat / Inventory 双方で分岐するため、片方で実装漏れが起きると「弾薬プールを Light で消費してしまう」「破損する」等の事故が起きやすい。Phase 2 完了時の Test Plan に以下を含めることでカバーする (実装 PR の Test Plan チェックリストに転記する):
  - Unit Test: `bIsRescue=true` 装備で射撃時、`UCallsignInventoryComponent::GetAmmoCount(Light)` が射撃前後で変化しない
  - Unit Test: `bIsRescue=true` 装備で射撃時、`UCallsignWeaponInstanceObject::GetDurabilityCurrent()` が射撃前後で変化しない (`bHasInfiniteDurability=true` の早期 return が機能している)
  - Unit Test: `bIsRescue=true` 装備で `Reload()` 呼出が no-op であり `OnReloaded` を broadcast しない
  - Unit Test: `bIsRescue=true` 装備で `IsBroken()` が常に false を返す (`DurabilityCurrent` をどんな負値にしても)
  - Integration Test: Rescue スロット装備のまま N 回 (例: 50) 射撃しても武器が破損せず弾切れも起きない
- `UCallsignInventoryComponent` を Pawn 所有にした §4.1 の判断が Phase 3 で覆る (PC 所有へ移譲) 場合、Pawn ごとに保持されている `WeaponSlot → UCallsignWeaponInstanceObject` のマッピングを PC 側へ移譲する作業が必要となる。Pawn 切替 (死亡 → リスポーン or 別 Pawn 操作) を扱うときに必ず再点検する。
- 弾薬プールを Pawn ごとに持たせる §11 の判断が Phase 3 で覆る (作戦持ち込み弾薬として PC や GameInstance 側へ移譲) 場合、`Inventory` 内 `TMap<ECallsignAmmoType, int32>` を別所有者へ移し、Combat 経路の `bUsesAmmoPool` 分岐における参照先も書き換える必要がある。
